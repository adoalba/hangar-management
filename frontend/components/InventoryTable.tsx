...
const InventoryTable: React.FC<InventoryTableProps> = ({ inventory, setInventory, onEdit, onPrint, t, user, token, addToast }) => {
  ...
  const isPenMode = pointerType === 'pen';
  const cellPadding = isPenMode ? 'p-2' : 'p-4'; // High Precision Mode for Pen: Denser grid
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-800 bg-brand-dark/40">
                  <th className={`${cellPadding} text-xs font-black text-white uppercase tracking-widest w-24`}>Tag</th>
                  <th className={`${cellPadding} text-xs font-black text-white uppercase tracking-widest`}>Component</th>
                <th className={`${cellPadding} text-xs font-black text-white uppercase tracking-widest`}>Identification</th>

                  {/* DYNAMIC COLUMNS BASED ON FILTER */}
                  {filterTag === TagColor.YELLOW && <th className={`${cellPadding} text-xs font-black text-yellow-500 uppercase tracking-widest`}>Expiration / TAT</th>}
              {filterTag === TagColor.RED && <th className={`${cellPadding} text-xs font-black text-rose-500 uppercase tracking-widest`}>Rejection Reason</th>}
                  {filterTag === TagColor.GREEN && <th className={`${cellPadding} text-xs font-black text-emerald-500 uppercase tracking-widest`}>Tech Report</th>}
            {filterTag === TagColor.WHITE && <th className={`${cellPadding} text-xs font-black text-brand-muted uppercase tracking-widest`}>Removal / Storage</th>}

                  <th className={`${cellPadding} text-xs font-black text-white uppercase tracking-widest`}>Location</th>
          <th className={`${cellPadding} text-xs font-black text-white uppercase tracking-widest text-right`}>Actions</th>
                </tr>
              </thead>
